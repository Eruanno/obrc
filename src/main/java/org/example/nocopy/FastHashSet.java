...
class FastHashSet {
    ...
    public void put(FileChannelReader fileChannelReader, int value) {
        int ptr = fileChannelReader.hashCode() & m_mask;
        TemperatureAggregator key = m_data[ptr];

        if (key == null) {
            put(new TemperatureAggregator(fileChannelReader.getCity(), value));
            return;
        }
        if (key.getCity().hashCode() == fileChannelReader.hashCode() && key.equals(fileChannelReader)) {
            key.add(value);
            return;
        }
